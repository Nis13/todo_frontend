...
import { mockedPost, mockedUsedDispatch, mockedUsedNavigate } from '../../setuptest';
...
describe('Given: useLogin Hook', () => {
  describe('When: useLogin is called', () => {
    it('Then: should mutate and trigger onSuccess', async () => {
      ...
      await waitFor(() => {
        expect(mockedPost).toHaveBeenCalledWith('/auth/login', userCredentialsDemoData);
  ...
  describe('When the mutation fails', () => {
    it('should set an error response', async () => {
      ...
      await act(async () => {
        await expect(result.current.handleLogin(userCredentialsDemoData)).rejects.toThrow(
          errorResponseDemo
        );
      });
